...
// headerSize is the OP header byte length (wire-protocol OpenSpec layout table).
const headerSize = 8

// Byte offsets inside the OP header (wire-protocol OpenSpec).
const (
	offHdrVersion = 0
	offHdrOpCode  = 2
	offHdrStatus  = 4
...
// DecodeHeader reads an 8-byte OP header from r. Per wire-protocol OpenSpec error
// matrix:
//
...
// directly and classifies the status itself. That path never reaches
// DecodeHeader, so the unconditional status check here is safe.
//
...
func DecodeHeader(r io.Reader) (uint16, OpCode, uint32, error) {
	...
	if status != 0 {
		return 0, 0, 0, fmt.Errorf("%w: opcode 0x%04x status=%d",
...
// decodeHeaderAllowStatus performs the wire-protocol OpenSpec header decode WITHOUT
...
// mapHeaderReadErr classifies the io.ReadFull error into the wire-protocol OpenSpec
