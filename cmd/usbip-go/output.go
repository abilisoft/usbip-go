...
// JSON output and every jsonlines event record (json-contracts OpenSpec stability
...
// responses are JSON-only (cli-interface OpenSpec) and therefore do not appear on
...
// typed struct guarantees "schema" is the first JSON key (json-contracts OpenSpec
...
// AttachAck writes the attach operation acknowledgement — cli-interface OpenSpec.
