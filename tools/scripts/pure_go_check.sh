...
if ! go list -mod=vendor -f '{{ if .CgoFiles }}{{ .ImportPath }}{{ end }}' ./... >"${tmpfile}"; then
...
if grep -rnE '(^|[[:space:]])import[[:space:]]+"C"' \
	--include='*.go' \
	--exclude-dir=.git \
	--exclude-dir=vendor \
	.; then
...
if ! find . -name '*.go' -not -path './.git/*' -not -path './vendor/*' -print0 | xargs -0 -r awk '
