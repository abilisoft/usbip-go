...
// the log line emitted by cmd/usbipd/run.go's "usbipd accepting
...
func TestDaemonRestartSessionsSurvive(t *testing.T) {
	...
	// cmd/usbipd exposes when --status is passed; our subprocess
...
// startDaemonSubprocess spawns cmd/usbipd at binary with --listen
...
// by cmd/usbipd/run.go and converts it into a RemoteEndpoint. Lets
...
// buildDaemonBinary compiles cmd/usbipd into t.TempDir so daemon
