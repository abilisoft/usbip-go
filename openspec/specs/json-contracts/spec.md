...
Port JSON views SHALL include `id`, `status`, `speed`, `remote`, `busid`, and `local_busid`.
...
- **WHEN** `list --remote`, `list --local`, or equivalent device rendering succeeds in JSON mode
