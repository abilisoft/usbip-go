...
The codec SHALL preserve clean EOF, wrap short reads with context, reject invalid fixed-width strings, ignore extra devlist trailing bytes with a warning, and surface padded-string truncation as advisory decode flags.
...
#### Scenario: Devlist has extra bytes
- **WHEN** all declared devices are decoded and bytes remain
...
The wire package SHALL maintain conformance fixtures from upstream captures and fuzz targets for protocol-critical decoders.
...
- **WHEN** fuzz targets generate malformed USB/IP frames
- **THEN** the decoder returns classified errors without panics or unbounded reads
