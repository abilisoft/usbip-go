...
| Kernel integration     | The dedicated free GitHub-hosted Ubuntu job verifies and boots a SHA-512-pinned Debian full-kernel image under QEMU because the Azure host omits required exporter/gadget modules. The guest loads USB/IP/gadget modules and mounts configfs before the Bazel-backed live suite. Host VM tooling, the guest image, and guest bootstrap downloads are narrowly scoped non-hermetic exceptions; missing capabilities fail the release gate. |
