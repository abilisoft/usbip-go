...
| SAST                   | `.github/workflows/codeql.yml` traces an uncached local Bazel build of the production binary through `make build-codeql`, then runs CodeQL `security-and-quality` on every push/PR + weekly cron. |
